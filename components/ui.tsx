...
export function StatusBadge({ status }: { status: string }) {
  let label = status;
  let tone: "danger" | "warning" | "positive" | "primary" = "primary";

  if (status === "PENDING" || status === "Kurye Aranıyor" || status === "Bekliyor" || status === "İncelemede" || status === "Kurye Bekleniyor") {
    label = "Kurye Bekleniyor";
    tone = "warning";
  } else if (status === "ASSIGNED" || status === "IN_TRANSIT" || status === "Yolda" || status === "Aktif") {
    label = "Yolda";
    tone = "primary";
  } else if (status === "DELIVERED" || status === "Teslim Edildi" || status === "Tamamlandı" || status === "Doğrulandı") {
    label = "Teslim Edildi";
    tone = "positive";
  } else if (status === "CANCELLED" || status === "Gecikiyor" || status === "Reddedildi" || status === "Vadesi Geçti") {
    label = "İptal Edildi";
    tone = "danger";
  }

  return <Badge tone={tone}><Clock3 className="mr-1 size-3" />{label}</Badge>;
}
